...
if __name__ == "__main__":
	...
	selected_pack_metadata = json.load(open(selected_pack_url + "/mmc-pack.json"))
	selected_pack_version = [x['version'] for x in selected_pack_metadata["components"] if x['cachedName'] == "Minecraft"][0]
	...
	while(True):
		...
		print("3) Remove mod")
		print("4) Remove mod, sort by name")
		try:
			choice = int(input(">>"))
		except:
			exit()
		if choice == 1:
			...
			buh = curse.modlist_manager.modlist.copy()
			buh.sort(key=(lambda x: x["name"].lower()))
			modlist = []
			for idx, entry in enumerate(buh):
				modlist.append(entry["id"])
				print("#{} {} {}".format(idx + 1, entry["name"], entry["filename"]))
			try:
				selected_mod = int(input(">>")) - 1
			except:
				exit()
